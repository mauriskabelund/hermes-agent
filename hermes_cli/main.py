...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # Desktop owns SSH runtime tokens at the machine-level Hermes root, even
    # when the spawned backend is scoped to a named profile.  Using the
    # profile home here rejects the path that Desktop intentionally created at
    # ``<root>/desktop-ssh`` and prevents every named profile from starting.
